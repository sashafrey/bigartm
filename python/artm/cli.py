...
def run_bigartm(args):
    ...
    # unique_tokens = artm.library.Library().LoadDictionary(os.path.join(batches_disk_path,
    # 'dictionary'))
    ...
    while not done:
        ...
        if done or (current_items_processed >= next_items_processed):
            ...
            print_diagnostic('processed %d items, perplexity = %f' % (
            current_items_processed, current_perplexity_score))
...
def main():
    ...
    parser.add_argument('--bigartm-path', default=os.path.expanduser('~/bigartm/'))
